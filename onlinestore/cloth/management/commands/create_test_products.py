    ...
    def handle(self, *args, **options):
        ...
        for category in categories:
            ...
            for key in product_templates.keys():
                if key in category_slug or category_slug in key:
                    template_key = key
                    break
            ...
            for template in templates:
                for color_name in ['Черный', 'Белый', 'Серый', 'Бежевый']:
                    ...
                    created_count += 1
                    self.stdout.write(f'  ✓ Создан товар: {product_name} (категория: {category.name})')

        self.stdout.write(self.style.SUCCESS(f'\nГотово! Создано товаров: {created_count}'))
        self.stdout.write(
            self.style.WARNING('\nОбратите внимание: Изображения нужно добавить вручную через админ-панель'))
